...
def arg_max_compliance(pix, sig_clusters, threshold_clusters):
    ...
    for i in range(len(sig_clusters)):
        difference = angle_between_vecs_numba(
            sig_clusters[i], pix) - threshold_clusters[i]
        if max_e < difference:
            max_e = difference
            max_i = i
    return max_e, max_i
...
def reference_clustering(HSI, threshold=0.90, clusters=[], value_mask_on=False, rgb_image=False):
    ...
    Параметры
        ...
        rgb_image : np.array
            ...
            где изображение хранится по строкам.
	'''
    ...
    for i in range(start_clust, amo_of_pix):
        ...
        max_difference, class_number = arg_max_compliance(
            HSI[i], signatures, thresholds)
    ...
    for i in range(len(signatures)):
        for col in range(3):
            cluster_mask_color[cluster_mask == i + 1,
                               col] = rgb_image[cluster_mask == i + 1, col].mean()
